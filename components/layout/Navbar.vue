...
const getNavbarHeight = () => {
  if (!common.value?.layout?.navbar?.height) return '83px';
  return isMobile.value
    ? common.value.layout.navbar.height.mobile || '83px'
    : common.value.layout.navbar.height.desktop || '68px';
};

const getLogoHeight = () => {
  if (!common.value?.layout?.navbar?.logo?.height) return '32px';
  if (isMobile.value) return common.value.layout.navbar.logo.height.mobile || '24px';
  if (isTablet.value) return common.value.layout.navbar.logo.height.tablet || '28px';
  return common.value.layout.navbar.logo.height.desktop || '28px';
};
